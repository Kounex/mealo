...
class _MealRandomizerState extends ConsumerState<MealRandomizer> {
  ...
  bool _hasNotExcludedTags(Meal meal) => this.widget.excludedTags.every(
    (tag) => !meal.tagUuids.contains(tag.uuid),
  );

  bool _hasIncludedIngredients(Meal meal) =>
      this.widget.includedIngredients.every(
        (ingredient) => meal.ingredients.any(
          (ingredientLink) => ingredientLink.uuidIngredient == ingredient.uuid,
        ),
      );

  bool _hasNotExcludedIngredients(Meal meal) =>
      this.widget.excludedIngredients.every(
        (ingredient) => meal.ingredients.every(
          (ingredientLink) => ingredientLink.uuidIngredient != ingredient.uuid,
        ),
      );
  ...
  bool _hasNotBeenEatenSince(Meal meal, List<RandomizedRun> randomizedRuns) {
    ...
    return randomizedMealsOfGivenMeal.every(
      (randomizedRun) => randomizedRun.updated.isBefore(hasNotBeenEatenSince),
    );
  }
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed:
                          !asyncCurrentRandomizedRun.isLoading &&
