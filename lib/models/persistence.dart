...
/// Design:
///
/// Data will always be served from the persistence layer. Initially entities
...
/// changes on the persistence layer. Therefore after fetching new entities
...
mixin Persistence<T extends BaseModel> on AutoDisposeAsyncNotifier<List<T>> {
  FutureOr<List<T>> init() async {
    _watcher();
    return _local();
  }

  void _watcher() async => PersistenceUtils.watch<T>().listen(
        (_) async {
  ...
  Future<List<T>> _local() async {
    List<T> entries = PersistenceUtils.where<T>().findAll();

    return entries;
  }
}

/// Same as [Persistence] but for singletons - for edge cases like [Settings]
/// where we "abuse" [Isar] to have a collection with only one entry (usually
/// not the use case) since we have the whole persistence cycle done and it
/// would be tedious to have something else in place for such edge cases
mixin SingletonPersistence<T extends BaseModel> on AutoDisposeAsyncNotifier<T> {
